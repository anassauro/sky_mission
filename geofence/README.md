...
Fortunately, we won't need to use calculus to prove this algorithm, just use vector product. If you're curious, see Dan Sunday's article. Let there be a polygon P with n sides and a point Q in the _xy_ plane. Imagine that you are traveling clockwise through a polygon, walking along its sides from one vertex to another. Let us draw a ray parallel to the _x_ axis leaving the point and going in the positive direction of the _x_ axis. Now, imagine that on your way around the perimeter you have a task: count how many times you cross the ray, adding +1 if you cross the ray from below to above (going up the _y-_axis) and -1 if you cross from above to below
